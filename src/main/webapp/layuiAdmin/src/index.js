  ...
  ,entryPage = function(fn){
    ...
    if(isIndPage || pathURL === '/user/login'){ //此处单独判断登入页，是为了兼容旧版（即未在 config.js 配置 indPage 的情况）
      ...
      if(setter.interceptor){
        ...
        if(!local[setter.request.tokenName]){
        ...
        }/*else if(sessionStorage.getItem("USER")==""||sessionStorage.getItem("USER")==null){
          return location.hash = '/user/login/redirect='+ encodeURIComponent(pathURL);
        	...
        	$.ajax({
        		...
        		async:false,
        		dataType:'json',
        		data:{"json":layui.data('layuiAdmin').access_token},
        		success:function(res){
        			layui.use(['admin', 'form', 'user'], function(){
        				  ...
        				  if(res.msg.status==1){
        					  parent.layer.msg("用户已被禁用，若有疑问请联系管理员");
        					  return location.hash = '/user/login/redirect='+ encodeURIComponent(pathURL);
        					  
        				  }else if(res.msg.password!=layui.data('layuiAdmin').user.password){
        					  parent.layer.msg("密码错误");
        					  return location.hash = '/user/login/redirect='+ encodeURIComponent(pathURL);
        				  }
